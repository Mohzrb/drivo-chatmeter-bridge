...
function pick(flat, keysOrRegexes) {
  for (const cand of keysOrRegexes) {
    if (cand instanceof RegExp) for (const [k,v] of Object.entries(flat)) if (cand.test(k) && isNonEmpty(v)) return v;
    else { const v = flat[normKey(cand)]; if (isNonEmpty(v)) return v; }
  } return undefined;
}
...
function extract(payload) {
  ...
  const author       = first(pick(flat, [/(\.|^)author(\.|$)/, /(\.|^)reviewer(\.|$)/, "review.author", "payload.author", "reviewer_name"]), "Reviewer");
