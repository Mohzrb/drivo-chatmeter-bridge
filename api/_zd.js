...
function ensureToken() {
  if (!process.env.ZD_TOKEN) {
    const err = new Error("Missing ZD_TOKEN environment variable");
    err.code = "NO_ZD_TOKEN";
    throw err;
  }
}
...
async function createOrUpdateFromChatmeter({ reviewId, subject, body, requester = "reviews@drivo.com", tags = ["chatmeter","review","google"], customFieldId }) {
  ...
  if (existing) {
    await axios.put(`${ZD_BASE}/tickets/${existing.id}.json`, { ticket: { comment: { body, public: true }, tags: allTags } }, { headers });
    return { action: "updated", id: existing.id, externalId };
  }

  const ticket = { subject, external_id: externalId, requester: { email: requester }, comment: { body, public: true }, tags: allTags };
  if (customFieldId) ticket.custom_fields = [{ id: Number(customFieldId), value: reviewId }];
