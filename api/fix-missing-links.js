...
} = require("../src/integrations/providers");
...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!ZD_SUB || !ZD_EMAIL || !ZD_TOK || !F_REVIEW) return res.status(500).send("Missing required Zendesk envs.");
    ...
    for (const t of tickets) {
      checked++;
      try {
        const tr = await jf(`https://${ZD_SUB}.zendesk.com/api/v2/tickets/${t.id}.json`, { headers: { Authorization: auth } });
        ...
        const aud = await jf(`https://${ZD_SUB}.zendesk.com/api/v2/tickets/${t.id}/comments.json?include=users`, { headers: { Authorization: auth } });
        ...
        if (!locationId) { const m = body.match(/\((\d{9,})\)\s*<\/div>/) || body.match(/\((\d{9,})\)/); if (m) locationId = m[1]; }

        const entry = getLocEntry(locationId);
        let link = null, aliasInfo = null;

        switch (provider) {
          case "YELP":       link = yelpUrlFromAlias(entry?.yelp); aliasInfo = entry?.yelp ? `Business (alias): ${entry.yelp}` : null; break;
          case "GOOGLE":     link = googleReviewsUrlFromConfig(entry); break;
          case "TRUSTPILOT": link = trustpilotUrlFromConfig(entry, globalEntry); break;
        }
